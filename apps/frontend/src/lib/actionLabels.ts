import { JsonValue } from '@fundifyhub/types';
...
/**
 * Type guard for checking if value is an object (not null/array)
 */
function isPlainObject(value: unknown): value is Record<string, unknown> {
  return typeof value === 'object' && value !== null && !Array.isArray(value);
}

export function formatMetadata(metadata: JsonValue | undefined): Array<{ key: string; value: string }> {
  if (metadata === null || metadata === undefined) return [];
  
  // If metadata is a string (legacy), try to parse JSON
  let meta: JsonValue = metadata;
  if (typeof metadata === 'string') {
    try { 
      meta = JSON.parse(metadata) as JsonValue; 
    } catch {
      meta = { info: metadata };
    }
  }
  ...
  if (!isPlainObject(meta)) {
    return [{ key: 'value', value: String(meta) }];
  }
  ...
  for (const k of Object.keys(meta)) {
    try {
      ...
      entries.push({ key: k, value: '[unable to format]' });
    }
  }
  return entries;
}
