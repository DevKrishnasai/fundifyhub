...
export default function RegisterPage() {
  ...
  const checkEmailExists = async (email: string) => {
    ...
    try {
  const data = await post<AvailabilityResponse>(BACKEND_API_CONFIG.ENDPOINTS.AUTH.CHECK_AVAILABILITY, { email })
  ...
  const checkPhoneExists = async (phone: string) => {
    ...
    try {
  const data = await post<AvailabilityResponse>(BACKEND_API_CONFIG.ENDPOINTS.AUTH.CHECK_AVAILABILITY, { phone })
  ...
  const sendEmailOTP = async () => {
    ...
    try {
      const res = await postWithResult<OTPSendResponse>(BACKEND_API_CONFIG.ENDPOINTS.AUTH.SEND_OTP, { email: step2Data.email })
  ...
  const sendPhoneOTP = async () => {
    ...
    try {
      const res = await postWithResult<OTPSendResponse>(BACKEND_API_CONFIG.ENDPOINTS.AUTH.SEND_OTP, { phone: step2Data.phoneNumber })
  ...
  const verifyOTP = async (type: 'EMAIL' | 'PHONE') => {
    ...
    try {
      const res = await postWithResult<OTPVerifyResponse>(BACKEND_API_CONFIG.ENDPOINTS.AUTH.VERIFY_OTP, { sessionId, otp })
  ...
  const resendOTP = async (type: 'EMAIL' | 'PHONE') => {
    ...
    try {
      const res = await postWithResult<OTPSendResponse>(BACKEND_API_CONFIG.ENDPOINTS.AUTH.SEND_OTP,
  ...
  const completeRegistration = async () => {
    ...
    try {
      ...
      const res = await postWithResult<RegisterResponse>(BACKEND_API_CONFIG.ENDPOINTS.AUTH.REGISTER, {
