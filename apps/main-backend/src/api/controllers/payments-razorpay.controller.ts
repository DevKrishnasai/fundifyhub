...
import { createRazorpayService } from '../../utils/razorpay-service';
...
// Validate Razorpay service on startup
...
export const createRazorpayOrderController = async (
  ...
  try {
    ...
    const orderResult = await razorpayService.createOrder(createOrderInput);
...
export const verifyRazorpayPaymentController = async (
  ...
  try {
    ...
    // Fetch payment details from Razorpay using service
    const fetchResult = await razorpayService.fetchPayment({ providerPaymentId: paymentId });
    ...
    // Type narrowing - after success check, we have the success response
    const amount: number = (fetchResult as any).amount || 0;
    const status: string = (fetchResult as any).status || 'unknown';
    const notes: Record<string, any> = (fetchResult as any).notes || {};
    const method: string | null = (fetchResult as any).method || null;
    
    logger.info(
      `Razorpay payment status: ${status}, amount: ₹${amount / 100}`
    );
    ...
    const amountNumeric = amount / 100;
