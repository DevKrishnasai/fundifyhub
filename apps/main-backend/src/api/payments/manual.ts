...
import { ROLES, PAYMENT_METHOD, PAYMENT_TYPE, EMI_STATUS, LOAN_STATUS } from '@fundifyhub/types';
...
async function hasLoanAccess(
  ...
  if (hasAnyRole({ roles: userRoles, districts: userDistricts }, [ROLES.DISTRICT_ADMIN, ROLES.STATE_ADMIN])) {
    const hasAccess = hasDistrictAccess({ roles: userRoles, districts: userDistricts }, loan.request.district.name);
